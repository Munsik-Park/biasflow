...
| Spec Simulator CI | `.github/workflows/spec-simulator.yml` + `engine/**` + `test/spec/**` + `test/engine/**` | The declaration-lint / simulation / digest-replay / flow-engine subject changes — `spec/**`, the `engine/*.mjs` modules, `test/spec/run.mjs`, `test/engine/run.mjs`, `test/spec/fixtures/flow-outcomes.mjs`, `docs/cycle-digest.jsonl`, or `.claude/hooks/check-autoflow-gate.sh` (the behavioral source `engine/gate.mjs` mirrors, hence a trigger path) — or the workflow's trigger paths, pinned `actions/checkout` / `actions/setup-node` commit SHAs or node version, or the advisory-enforcement framing change (issues #2, #4) | Orchestrator AI |
